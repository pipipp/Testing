...
class SocketFileSync(object):
    ...
    def __init__(self, local_host_ip, other_host_ip, file_directory='Socket_Files'):
        ...
        self.socket_timeout_time = 60  # 服务端和客户端的Socket超时时间，单位秒
        self.automatic_sync_time = 60  # 客户端自动启动同步的周期时间，单位秒

        self.maximum_transfer_size = 1073741824  # 文件传输上限1G，单位b
        self.buffer_size = 1024000  # Socket buffer size，单位b
                ...
                while True:
                    ...
                    while True:
                        ...
                        if 'file transfer done'.encode() in socket_data:
                            print('*'*20)
                            # print('Last socket: {}'.format(socket_data.decode()))
                            print('*'*20)
                            break
                    ...
                    print('收到 file size length: {}'.format(len(data_content)))
                    # 一次性写入文件，防止文件粘连
                        ...
                        if need_sync_files:
                            ...
                            for each_file in need_sync_files:  # 循环传输每一个文件
                                ...
                                while True:
                                    ...
                                    self.send_socket_info(handle=client, side='client', msg='file transfer done')
                                    print('file size length: {}'.format(size))
