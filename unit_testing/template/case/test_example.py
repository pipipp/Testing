...
跳过用例方法：使用装饰器
@unittest.skip(reason)                       # 直接指定skip，reason用于描述跳过的原因
@unittest.skipIf(condition, reason)          # 当condition为True时，跳过用例
@unittest.skipUnless(condition, reason)      # 当condition为True时，执行用例
"""
import sys
import unittest
...
class UnitTestExample(unittest.TestCase):
    ...
    # 跳过指定的用例
    @unittest.skip('Use the "skip" method to skip the case test for test_case_2')
